...
Storage backend is selected by `TTS_PREVIEW_STORAGE`
(`app/core/config/static.py`, read once at import — not a live Redis toggle),
with two more env vars governing the GCS backend:

| Var | Default | Purpose |
|---|---|---|
| `TTS_PREVIEW_STORAGE` | `"local"` | `"local"` or `"gcs"` — anything else fails startup |
| `TTS_PREVIEW_GCS_BUCKET` | `""` | GCS bucket name for previews; **required** (startup failure if empty) when `TTS_PREVIEW_STORAGE=gcs` |
| `TTS_PREVIEW_PUBLIC_BASE_URL` | `""` | Base URL previews are served from — applies to **both** modes. For `gcs`, the bucket's public base URL (e.g. `https://storage.googleapis.com/<bucket>`) — **required** in gcs mode. For `local`, this API's own public origin (e.g. `http://localhost:8931` in dev). |
...
Both backends store at the same relative path shape:
`{provider}/{voice_id}/{language}-{key}.wav`, plus `manifest.json`, all under
the `tts-previews/` prefix (on disk: `var/tts-previews/...`). Local mode serves
that directory itself at `/tts-previews` (the mount uses `check_dir=False`; the
directory is created in the FastAPI lifespan, so no filesystem I/O happens at
import time) and local WAV/manifest writes are atomic (temp file +
`os.replace`) so a concurrently served file is never partial. When
`TTS_PREVIEW_PUBLIC_BASE_URL` is unset, local mode returns root-relative URLs
(`/tts-previews/...`) — fine when the caller shares this API's origin. That
assumption breaks for split-origin setups (a frontend dev proxy that only
forwards specific path prefixes, or any deployment where the API isn't served
from `/`); setting `TTS_PREVIEW_PUBLIC_BASE_URL` makes local mode return
absolute URLs, the same shape GCS mode always uses.
...
— it isn't derived on read. To force full regeneration: delete the stored
`manifest.json` (local: `rm var/tts-previews/manifest.json`; gcs: delete the
...
before pointing `TTS_PREVIEW_STORAGE` at it in prod, or every preview URL will
403 for end users. This is an operational precondition the app doesn't check.
The manifest lives in that same public location by design — it contains only
public preview URLs and content-key hashes.
