...
async def test_grandfathered_cross_merchant_pin_still_dials(pool, monkeypatch):
    """Existing cross-merchant pins keep working (warn, never block)."""
    ...
    assert any("grandfathered cross-merchant pin" in w for w in warnings)
    # PII stays out of logs: the warning carries the number id, not the number
    assert not any(foreign.number in w for w in warnings)
